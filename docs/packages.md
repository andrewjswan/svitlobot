...
!!! danger "Важливо: Оберіть правильний пакет!"
    ...
    * **`interval_esp8266.yaml`** — якщо у вас плата на базі **ESP8266**.
...
* **[espnow_heatbeat.yaml](https://github.com/andrewjswan/svitlobot/blob/main/packages/espnow_heatbeat.yaml)** — Пакет, який об'єднує кілька контролерів у відмовостійку групу через протокол ESPNow для автоматичного вибору одного «Майстра». Він гарантує, що критичні дії (наприклад, надсилання сповіщень у **Світло**Бот) виконує лише один пристрій у мережі, запобігаючи дублюванню команд і забезпечуючи миттєве перемикання на резервний вузол у разі збою основного. [Детальніше](espnow-failover.md).
