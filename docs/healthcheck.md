...
1. **Пристрій:** Раз у період відправляє сигнал (heartbeat) на сервер.
...
2. Ядро **Svitlo**Bot
3. Моніторинг доступності (**Health**Checks)
...
!!! example annotate "HealthChecks | Custom URL"
    ...
    ```
1. Базові налаштування
2. Моніторинг доступності (**Health**Checks)
...
!!! example annotate "HealthChecks"
    ...
    packages:
      remote_package:
        ...
        refresh: 1s
    ```
1. Специфічні параметри платформи **ESP32** для **ESP8266** використовуйте **esp8266.yaml**
...
*   **Period (Інтервал):** встановіть `1 minute`. Це частота, з якою пристрій надсилає сигнал.
...
!!! example annotate "Telegram Bot"
    ...
    **POST** | [https://api.telegram.org/bot88888888888:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA/sendMessage](https://core.telegram.org/bots)
    ...
    ```
1. Ідентифікатор користувача, каналу або чату в Telegram, який отримуватиме повідомлення.
2. Ідентифікатор користувача, каналу або чату в Telegram, який отримуватиме повідомлення.
